...
(function (n, r, f) {
    ...
    /**
     * Returns the last fragment which does something.  This eliminates
     * whitespace and comments.
     *
     * @return {?prettyJs~resultBit}
     */
    Result.prototype.getPreviousWorkingFragment = function () {
        var code, i;

        for (i = this.fragments.length - 1; i >= 0; i -= 1) {
            code = this.fragments[i].code;

            if (code !== 'SPACE' && code !== 'INDENT' && code !== 'NEWLINE' && code !== 'STATEMENT_COMMENT' && code !== 'COMMENT_WHITESPACE' && code !== 'BLOCK_COMMENT') {
                return this.fragments[i];
            }
        }
    ...
    function convertString(result, content) {
        ...
        converted = converted.replace(quote, '\\' + quote);
    ...
    /**
     * This may change a token from a keyword into an identifier depending on
     * the tokens immediately before it.
     *
     * For instance, there's the times you use keywords as properties:
     *     x.toString();
     *     y.default = "text";
     *
     * They should be processed as identifiers instead.
     * Hand off the token processing to another function based on
     * the token's content.
     *
     * @param {Object} rules Map of exact string to processor
     * @param {prettyJs~processor} defaultKeywordProcessor
     * @param {prettyJs~processor} defaultIdentifierProcessor
     * @return {prettyJs~processor}
     */
    function processKeyword(rules, defaultKeywordProcessor, defaultIdentifierProcessor) {
        var keywordProcessor;

        keywordProcessor = processByContent(rules, defaultKeywordProcessor);

        return function (result, token) {
            var fragment;

            fragment = result.getPreviousWorkingFragment();

            if ((fragment && fragment.content === '.') || result.getContextCode() === 'BRACE') {
                token.type = 'IDENTIFIER_NAME';

                return defaultIdentifierProcessor(result, token);
            }

            return keywordProcessor(result, token);
        };
    }
    ...
    function tokenPunctuatorBraceClose(result, token) {
        ...
        // If in ternary, remove that context
        if (result.getContextCode() === 'TERNARY') {
            result.removeContext();
        }
        ...
        if (context === 'CATCH' || context === 'ELSE' || context === 'FOR' || context === 'FUNCTION' || context === 'IF' || context === 'SWITCH') {
    ...
    function tokenPunctuatorColon(result, token) {
        ...
        if (context !== 'TERNARY') {
            ...
            if (result.options.quoteProperties === true) {
                // Force quotes
                if (result.getType() === 'IDENTIFIER_NAME') {
    ...
    function tokenSemicolon(result) {
        ...
        if (oldContext === 'IF' || oldContext === 'ELSE' || oldContext === 'FOR' || oldContext === 'TERNARY' || oldContext === 'VAR') {
            result.removeContext();
        }
    ...
    processors = {
        ...
        KEYWORD: processKeyword(keywordContentProcessors, tokenCopyAndSpace, tokenCopyAndSpace),
